...
const BillForm = ({ onSuccess }) => {
  ...
  const [billData, setBillData] = useState({
    ...
    items: prefilled.length
      ? prefilled.map((it) => ({
        _id: it._id,                // ✅ Required for backend item lookup
        itemName: it.name,         // Optional (can help in UI/debugging)
          ...
          <div className="flex flex-col md:flex-row md:space-x-4">
            ...
            <div className="w-full md:w-1/2">
              ...
              <input
                ...
                value={billData.commissionPercent}
                onChange={(e) =>
                  setBillData((prev) => ({
                    ...prev,
                    commissionPercent: Number(e.target.value),
                    commissionAmount:
                      (Number(e.target.value) / 100) * Number(prev.totalAmount || 0),
                  }))
                }
